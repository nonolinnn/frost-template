...
這個 PRD 執行順利，fr-008 預先提升的 selectedWalletIndex state 讓 fr-009 無需重構。Build 乾淨，zero TypeScript errors。

**事後發現與修正：** 手動測試時發現 balance check 被實作成硬擋，導致餘額為零時無法進入簽名流程，與 README 描述不符。修正為警告設計。

**設計判斷：** 前端 balance check 的職責是 UX 提示，不是安全守衛——真正的防線在 Solana 網路層，餘額不足的交易在廣播時會被網路拒絕，前端無法繞過這層保護。在 production 環境，硬擋是合理的 UX 優化（避免用戶等完簽名才發現廣播失敗）。但在 demo 情境下，faucet rate limit 是外部限制，硬擋會使核心協議流程完全無法展示，取捨上選擇警告。這也是 self-review 的邊界：功能層面正確，但 demo flow 的行為預期需要 human 實際跑過才能驗證。
